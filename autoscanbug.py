...
def get_target_url():
    while True:
        user_input = input("Silahkan masukan Target URL (Contoh: https://example.com): ").strip()
        if not user_input:
            print("URL tidak boleh kosong. Silakan coba lagi.")
            continue

        # astika URL dimulai dengan http:// atau https://
        ...
        try:
            ...
            response.raise_for_status()  # Menimbulkan HTTPError untuk respons yang buruk
            print("Successfully connected to the URL.")
            return user_input
        except requests.exceptions.MissingSchema:
            print(
                "Format URL tidak valid. Harap sertakan 'http://' atau 'https://'. Coba ulangi."
            )
        except requests.exceptions.HTTPError as e:
            print(
                f"HTTP error {response.status_code} terjadi saat connecting ke URL: {str(e)}"
            )
        except requests.exceptions.ConnectionError:
            print(
                "Gagal connecting ke URL. Harap periksa koneksi atau URL Anda dan coba lagi."
            )
        except requests.exceptions.RequestException as e:
            print(f"Terjadi error saat connecting ke URL: {str(e)}")
...
def check_sqli(url):
    try:
        ...
        patterns = [
            ...
            r"java\.sql\.SQLException: Column tidak ditemukan",
            ...
            r"Warning: mysqli_.*",
            r"SQLSTATE\[42S02\]: Base table or view tidak ditemukan",
            ...
            r"SQLSTATE\[42S22\]: Column tidak ditemukan",
            r"ORA-00932",
            ...
            r"ERROR: relation \".*\" does not exist",
            r"SQLSTATE\[42S12\]: Column tidak ditemukan",
            r"ORA-02291",
            ...
            r"SQLSTATE\[42S02\]: Base table or view tidak ditemukan: 1146",
            ...
            r"SQLSTATE\[IM002\]: Data source name tidak ditemukan",
            r"java\.lang\.ArrayIndexOutOfBoundsException:",
            r"SQLSTATE\[42S12\]: Column tidak ditemukan: 1054",
...
def check_rce(url):
    try:
        ...
        # Periksa setiap pola terhadap konten respons
        for regex in rce_regex:
            if regex.search(response.text):
                return True  #Pola RCE terdeteksi
        return False  # Tidak ada pola RCE yang terdeteksi

    except Exception as e:
        print("Error occurred during RCE check:", e)
        return False  # Asumsikan tidak ada RCE jika terjadi kesalahan
...
#         response.raise_for_status()
#         
#         # dump File yang di gunakan untuk penguji
...
def check_rfi(url, payload="README.md"): # yang tadinya payload_file payload menjadi payload
    ...
    try:
        ...
        rfi_response = requests.get(url, params={"file": payload_content})

        if rfi_response.status_code == 200 and is_rfi_detected(rfi_response.text):
            test_url = url + "/"
            test_response = requests.get(test_url)
            if test_response.status_code == 200 and "tools" in test_response.text:
                return (
                    True,
                    "Remote File Inclusion (RFI): Possible RFI vulnerability detected.",
                )
...
def is_rfi_detected(response_text):
    ...
    rfi_keywords = ["tools V2.0", "RFI Detected", "Remote File Inclusion"]
    return any(keyword in response_text for keyword in rfi_keywords)
...
def check_unvalidated_redirects(url, file="README.md"):
    try:
        ...
        if "Location" in response.headers and response.status_code in [
            ...
            # Search for redirect indicators in the response text
            if re.search(r"Unvalidated Redirects", response.text, re.IGNORECASE):
                with open(file, "r") as file:
                    redirect_url = file.read()

                # Test redirection with local payload
                redirect_response = requests.get(url, params={"redirect": redirect_url})
                if "Redirect Successful" in redirect_response.text:
                    return (
                        True,
                        "Unvalidated Redirects: Possible unvalidated redirects detected.",
                    )
    except requests.RequestException:
        pass
...
def collect_urls(target_url, max_urls=500, num_threads=10, session=None):
    ...
    def process_url(current_url):
        ...
        try:
            ...
            logging.error(f"Request Exception untuk URL: {current_url}, Error: {e}")
        except Exception as e:
            logging.error(f"Terjadi eror pada URL: {current_url}, Error: {e}")
    ...
    with tqdm(
        ...
        desc="Mengumpulkan URL", # Collecting URLs
...
def save_vulnerable_urls(vulnerable_urls):
    """Tambahkan vulnerabilities URL yang ditemukan ke sebuah file."""
    with open("vulnerable_urls.txt", "a") as file:
        for url in vulnerable_urls:
            file.write(url + "\n")
    print_info("Hasil scanning telah ditambahkan ke 'vulnerable_urls.txt'.")
...
def print_warning(message):
    """Print warning menggunakan warna cyan."""
    print_colorful("\n[Bingo] " + message, Fore.CYAN)


def print_error(message):
    """Print error menggunakan warna red."""
    print_colorful("[Error] " + message, Fore.RED)


def print_info(message):
    """Print information menggunakan warna magenta."""
    print_colorful("[Info] " + message, Fore.MAGENTA)


# def collect_urls_from_file(file_path):
#     """Kumpulkan URL dar path file yang diberikan."""
#     try:
#         with open(file_path, "r") as file:
#             websites = file.read().splitlines()
#         return websites
#     except FileNotFoundError:
#         print_error(f"File '{file_path}' tidak ditemukan.")
#         return []
...
def get_target_url():
    """Minta pengguna untuk memasukkan URL dan parse  untuk memastikannya menyertakan scema."""
    target_url = input("Masukkan URL target untuk scanning vulnerabilities: ")
...
def main():
    while True:
        print(COLOR.LIGHT_CYAN + "+=====================+" + COLOR.RESET + COLOR.PINK + STYLE.BOLD + " AutoScanBug. " + COLOR.RESET + COLOR.LIGHT_CYAN + "+========================+" + COLOR.RESET)
        print(COLOR.LIGHT_CYAN + "+-------------------------------------------------------------+" + COLOR.RESET) 
        print(COLOR.LIGHT_CYAN + "+" + COLOR.RESET + COLOR.YELLOW + "                  Pilih Metode Pencarian.                    " + COLOR.RESET + COLOR.LIGHT_CYAN + "+" + COLOR.RESET)
        print(COLOR.LIGHT_CYAN + "+-------------------------------------------------------------+" + COLOR.RESET)
        print(COLOR.YELLOW + STYLE.BOLD + "1." + STYLE.RESET + COLOR.RESET + COLOR.GREEN + STYLE.BOLD + " Url Domain Tunggal." + STYLE.RESET + COLOR.RESET + COLOR.LIGHT_CYAN + "                                        +" + COLOR.RESET)
        print(COLOR.YELLOW + STYLE.BOLD + "2." + STYLE.RESET + COLOR.RESET + COLOR.GREEN + STYLE.BOLD + " Metode List Url Domain (" + STYLE.RESET + COLOR.RESET + COLOR.YELLOW + "urldomain.txt" + STYLE.RESET + COLOR.GREEN + STYLE.BOLD + ")." + STYLE.RESET + COLOR.RESET + COLOR.LIGHT_CYAN + "                    +" + COLOR.RESET)
        print(COLOR.LIGHT_CYAN + "+=============================================================+" + COLOR.RESET)
        print(COLOR.YELLOW + STYLE.BOLD + "B. Kembali ke Halaman Menu." + STYLE.RESET + COLOR.RESET + COLOR.LIGHT_CYAN + "                                   +" + COLOR.RESET)
        print(COLOR.LIGHT_CYAN + "+=============================================================+" + COLOR.RESET)
        choice = input(COLOR.LIGHT_CYAN + "[?] Masukkan pilihan: " + COLOR.RESET)
        
        if choice == "1":
            target_url = get_target_url()
            urls = collect_urls(target_url)
            print_info(f"Ditemukan {len(urls)} URL untuk di-scan.")
            vulnerable_urls = scan_urls(urls)
            save_vulnerable_urls(vulnerable_urls)
            break
        elif choice == "2":
            file_path = input("Masukkan path file list txt yang berisi websites: ")
            websites = collect_urls_from_file(file_path)
            if not websites:
                print_error("Tidak ada websites yang dimuat dari file list.")
                continue
            print_info(f"Memuat {len(websites)} websites dari file list.")
            vulnerable_urls = scan_websites(websites)
            save_vulnerable_urls(vulnerable_urls)
            break
        elif choice.lower() == "b":
            print_info("Kembali ke Halaman Menu.")
            break
        else:
            print_error("Pilihan Tidak Valid. Silakan Pilih Opsi 1, 2, atau B.")

def scan_urls(urls):
    print_info("Scanning URL Yang Dikumpulkan Untuk Searching VUlnerability...")
    vulnerable_urls = set()
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in tqdm(
            ...
            try:
                ...
                print_error(f"Terjadi Error Saat Scanning URL: {e}")
    print_info("Scanning selesai!")
    return vulnerable_urls

def scan_websites(websites):
    print_info("Scanning websites dari file list...")
    vulnerable_urls = set()
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(collect_urls, website) for website in websites]
        # Gunakan tqdm.tqdm(...) karena Anda mengimpor tqdm secara keseluruhan
        # for future in tqdm.tqdm(
        for future in tqdm(
            concurrent.futures.as_completed(futures),
            total=len(futures),
            ...
            try:
                ...
                print_error(f"Terjadi Error Saat Scanning Websites: {e}")
    print_info("Scanning selesai!")
    return vulnerable_urls
